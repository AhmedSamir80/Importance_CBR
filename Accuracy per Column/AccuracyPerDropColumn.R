AccuracyPerDropColumn<- function(TimeStamp)
{
...
TwtCorpus = Tweets_corpus
UsersData = UsersData
UseAcc = "all" 
Model_name = "RF"
linear = TRUE
tuned = FALSE #options for SVM
Stemmed = TRUE
UseWords = "Non-sparce"
...
TwtDataSet = cbind(TwtDataSet, as.matrix(dtm)) 
  ...
  Results = data.frame("Column" = character() , "Accuracy" = numeric() )
  AllCOlumns = colnames(TwtDataSet )
  for(columnname in colnames(TwtDataSet ))
  {
    if(columnname == "Class")
    {
      next
    }
    x2 <- AllCOlumns[!AllCOlumns %in% columnname]
 
    print(columnname)   
  id_train <- sample(nrow(TwtDataSet),nrow(TwtDataSet)*0.7)
  train_accounts = TwtDataSet[id_train,x2]
  test_accounts = TwtDataSet[-id_train,x2]
  ...
  library(caret)
  CM<- confusionMatrix(pred, test_accounts$Clas, positive = NULL, dnn = c("Prediction", "Reference"))
  TN = CM$table[1,1];  TP = CM$table[2,2]
  FP= CM$table[1,2];   FN = CM$table[2,1]
  
  Acc = (TP+TN) / (TP+TN+FP+FN) 
  print(Acc)
  Results = rbind(Results,c(columnname,Acc))
  }
  write.csv(Results,'AccuracyPerDrop.csv')
  	id_train <- sample(nrow(TwtDataSet),nrow(TwtDataSet)*0.5)
    train_accounts = TwtDataSet[id_train,]
	test_accounts = TwtDataSet[-id_train,]
  CaseBasedReasoningfunction(forest,'AccuracyPerDrop.csv',train_accounts,test_accounts)
}
...
AnnotateWords <- function(dtm)
{
  ...
  a2 <-NLP::annotate(words, list(sent_token_annotator, word_token_annotator))
  pos_tag_annotator <-Maxent_POS_Tag_Annotator()
  aWords <-subset(NLP::annotate(words, pos_tag_annotator, a2), type == "word")
