CreateVariableImpotranceWithPIMP<- function(TimeStamp)
{
  ...
  TwtDataSet = cbind(TwtDataSet, as.matrix(dtm)) 
    ...
    #using random sampling with 100%
    #print(nrow(TwtDataSet))
    id_train <- sample(nrow(TwtDataSet),nrow(TwtDataSet)*1)
    ...
    forest <- randomForest(Class ~ ., data = train_accounts,ntree=RFTrees,MaxNodes = MaxNodes,importance = TRUE, proximity=TRUE)
    names(test_accounts) <- make.names(names(test_accounts))
    library(caret)
    library(vita)
    mypimp = PIMP( train_accounts,UsersData$Class,forest, parallel=FALSE)
    #print(mypimp)
    write.csv(mypimp$VarImp,'importance with PIMP VarImp.csv')
	id_train <- sample(nrow(TwtDataSet),nrow(TwtDataSet)*0.5)
    train_accounts = TwtDataSet[id_train,]
	test_accounts = TwtDataSet[-id_train,]
    CaseBasedReasoningfunction(forest,'importance with PIMP VarImp.csv',train_accounts,test_accounts)
